...
export default {
  ...
  created() {
    this.id = this.$route.params.approvalId
    this.getCourse()
    this.getTextbook()
  },
  methods: {
    /* 获取课程 */
    getCourse() {
      approval.getDetails(this.id).then(res => {
        console.log(res)
        this.course = res.data.openingPlan.openingPlanDetails
      })
    },
    getTextbook() {
      approval.getTextbook(this.id).then(res => {
    ...
    close() {
      this.$confirm('当前数据将不会保存,确认关闭？').then(() => {
        ...
        this.$router.push({ path: '/textbookSubscription/approval' })
      })
    },
    /* 确认按钮按钮 */
    submit() {
      ...
      approval.reSubmit(this.id, approvalInsertDTO).then(res => {
        this.$store.dispatch('tagsView/delView', this.$router.currentRoute)
        this.$router.push({ path: '/textbookSubscription/approval' })
