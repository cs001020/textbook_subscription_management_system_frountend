...
export default {
  ...
  data() {
    return {
      ...
      title: '修改头像',
      options: {
        img: `https://kodo.warframe.top/${store.getters.avatar}`, // 裁剪图片的地址
  ...
  methods: {
    ...
    beforeUpload(file) {
      if (file.type.indexOf('image/') === -1) {
        this.$message({ type: 'error', message: '文件格式错误，请上传图片类型,如：JPG，PNG后缀的文件。' })
      } else {
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        uploadAvatar(formData).then(response => {
          this.open = false
          this.options.img = `https://kodo.warframe.top/${response.msg}`
          store.dispatch('user/resetAvatar', response.msg)
    ...
    // 关闭窗口
    closeDialog() {
      this.options.img = `https://kodo.warframe.top/${store.getters.avatar}`
      this.visible = false
