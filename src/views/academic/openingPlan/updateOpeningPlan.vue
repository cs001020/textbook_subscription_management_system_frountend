<template>
  <el-dialog title="修改开课计划" :visible.sync="open">
...
export default {
  ...
  methods: {
    show(id) {
      openingPlan.getPlanById(id).then(({ data }) => {
        this.teacher = data.teacher
        this.form.id = data.id
        this.form.teacherId = data.teacher.id
        this.selectedGrade = [parseInt(data.secondaryCollege.id), parseInt(data.major.id), parseInt(data.grade.id)]
        this.form.teachingGroupId = data.teachingGroup.id
        this.form.secondaryCollegeId = data.secondaryCollege.id
        this.form.majorId = data.major.id
        this.form.gradeId = data.grade.id
        this.form.openingPlanDetailDTOList = data.openingPlanDetails
        this.open = true
      })
    ...
    /* 最终提交按钮 */
    doUpdate() {
      openingPlan.updateOpeningPlan(this.form.id, this.form).then(res => {
        this.$emit('ok')
        this.open = false
      })
    }
